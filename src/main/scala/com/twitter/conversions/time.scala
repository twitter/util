...
object time {
  final private val NANOS = 1000L * 1000L * 1000L
  class RichWholeNumber(wrapped: Long) {
    def seconds = new Duration(wrapped * NANOS)
    def second = seconds
    ...
    def minutes = new Duration(wrapped * NANOS * 60)
    def minute = minutes
    def hours = new Duration(wrapped * NANOS * 60 * 60)
    def hour = hours
    def days = new Duration(wrapped * NANOS * 60 * 60 * 24)
    def day = days
  }
