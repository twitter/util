 ...
 */
case class BelowThroughputException(elapsed: Duration, currentBps: Double, expectedBps: Double)
    extends TimeoutException(s"current bps $currentBps below minimum $expectedBps")
