...
trait TimeLikeSpec[T <: TimeLike[T]] extends AnyFunSuite with ScalaCheckDrivenPropertyChecks {
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should behave like boxed doubles") {
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should complementary diff") {
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should complementary compare") {
    for (a <- vs; b <- vs) {
      val x = a compare b
      val y = b compare a
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should commutative max") {
    for (a <- vs; b <- vs)
      assert((a max b) == (b max a))
  }

  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should commutative min") {
    for (a <- vs; b <- vs)
      assert((a min b) == (b min a))
  }

  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should handle underflows") {
    assert(fromNanoseconds(Long.MinValue) - 1.nanosecond == Bottom)
    assert(fromMicroseconds(Long.MinValue) - 1.nanosecond == Bottom)
  }

  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should handle overflows") {
    assert(fromNanoseconds(Long.MaxValue) + 1.nanosecond == Top)
    assert(fromMicroseconds(Long.MaxValue) + 1.nanosecond == Top)
  }

  test(
    "Top, Bottom, Undefined, Nanoseconds(_), Finite(_): Nanoseconds(_) extracts only finite values, in nanoseconds") {
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_): Finite(_) extracts only finite values") {
    for (t <- Seq(Top, Bottom, Undefined))
      assert(t match {
        case Finite(_) => false
        case _ => true
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) should roundtrip through serialization") {
    for (v <- vs) {
  ...
  test("Top, Bottom, Undefined, Nanoseconds(_), Finite(_) has correct isZero behaviour") {
  ...
  test("Top should be impermeable to finite arithmetic") {
    assert(Top - 0.seconds == Top)
    assert(Top - 100.seconds == Top)
    assert(Top - Duration.fromNanoseconds(Long.MaxValue) == Top)
  }

  test("Top should become undefined when subtracted from itself, or added to bottom") {
    assert(Top - Duration.Top == Undefined)
    assert(Top + Duration.Bottom == Undefined)
  }

  test("Top should not be equal to the maximum value") {
    assert(fromNanoseconds(Long.MaxValue) != Top)
  }

  test("Top should always be max") {
  ...
  test("Top should greater than everything else") {
    assert(fromSeconds(0) < Top)
    assert(fromFractionalSeconds(Double.MaxValue) < Top)
    assert(fromNanoseconds(Long.MaxValue) < Top)
  }

  test("Top should equal to itself") {
    assert(Top == Top)
  }

  test("Top should more or less equals only to itself") {
  ...
  test("Top should Undefined diff to Top") {
    assert((Top diff Top) == Duration.Undefined)
  }

  test("Bottom should be impermeable to finite arithmetic") {
  ...
  test("Bottom should become undefined when added with Top or subtracted by bottom") {
    assert(Bottom + Duration.Top == Undefined)
    assert(Bottom - Duration.Bottom == Undefined)
  }

  test("Bottom should always be min") {
    assert((Bottom min Top) == Bottom)
    assert((Bottom min fromNanoseconds(0)) == Bottom)
  }

  test("Bottom should less than everything else") {
    assert(Bottom < fromSeconds(0))
    assert(Bottom < fromNanoseconds(Long.MaxValue))
    assert(Bottom < fromNanoseconds(Long.MinValue))
  }

  test("Bottom should less than Top") {
    assert(Bottom < Top)
  }

  test("Bottom should equal to itself") {
    assert(Bottom == Bottom)
  }

  test("Bottom should more or less equals only to itself") {
  ...
  test("Bottom should Undefined diff to Bottom") {
    assert((Bottom diff Bottom) == Duration.Undefined)
  }

  test("Undefined should be impermeable to any arithmetic") {
  ...
  test("Undefined should Undefined on diff") {
  ...
  test("values should reflect their underlying value") {
    val nss = Seq(
  ...
  test("inSeconds should equal inLongSeconds when in 32-bit range") {
    val nss = Seq(
  ...
  test("inSeconds should clamp value to Int.MinValue or MaxValue when out of range") {
  ...
  test("rounding should maintain top and bottom") {
    assert(Top.floor(1.hour) == Top)
    assert(Bottom.floor(1.hour) == Bottom)
  }

  test("rounding should divide by zero") {
  ...
  test("rounding should deal with undefineds") {
  ...
  test("rounding should round to itself") {
  ...
  test("from* should never over/under flow nanos") {
  ...
  test("from* should overflow millis") {
  ...
  test("from* should underflow millis") {
...
class TimeFormatTest extends AnyFunSuite {
  test("TimeFormat should format correctly with non US locale") {
  ...
  test("TimeFormat should set UTC timezone as default") {
    val format = "HH:mm"
    val timeFormat = new TimeFormat(format)
    assert(timeFormat.parse("16:04").format(format) == "16:04")
  }

  test("TimeFormat should set non-UTC timezone correctly") {
    val format = "HH:mm"
...
class TimeTest
  ...
  test("Time should work in collections") {
  ...
  test("Time withCurrentTimeFrozen") {
    val t0 = new Time(123456789L)
  ...
  test("Time fromFractionalSeconds") {
