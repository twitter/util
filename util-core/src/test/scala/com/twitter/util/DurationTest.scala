...
class DurationTest extends AnyFunSuite with DurationOps with TimeLikeSpec[Duration] {
  test("Duration *") {
  ...
  test("Duration moreOrLessEquals") {
    assert(10.seconds.moreOrLessEquals(9.seconds, 1.second))
    assert(10.seconds.moreOrLessEquals(11.seconds, 1.second))
    assert(!10.seconds.moreOrLessEquals(8.seconds, 1.second))
    assert(!10.seconds.moreOrLessEquals(12.seconds, 1.second))
  }

  test("Duration inTimeUnit") {
  ...
  test("Duration toString should display as sums") {
    assert((9999999.seconds).toString == "115.days+17.hours+46.minutes+39.seconds")
  }

  test("Duration toString should handle negative durations") {
    assert((-9999999.seconds).toString == "-115.days-17.hours-46.minutes-39.seconds")
  }

  test("Duration convert java.time.Duration") {
  ...
  test("Duration reject obvious human impostors") {
    Seq(
      "",
      "++1.second",
      "1. second",
  ...
  test("Top should be scaling resistant") {
