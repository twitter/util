...
class Eval(target: Option[File]) {
  ...
  lazy val impliedClassPath: List[String] = {
    ...
    def getCurrentClassPath(cl: ClassLoader, acc: List[List[String]] = List.empty): List[String] = {
      val cp = cl match {
      ...
      cl.getParent match {
        case null => (cp :: acc).flatten
        case parent => getCurrentClassPath(parent, cp :: acc)
      }
    }

    val currentClassPath = getCurrentClassPath(this.getClass.getClassLoader)
