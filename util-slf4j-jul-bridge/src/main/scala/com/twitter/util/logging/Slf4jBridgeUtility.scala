...
/**
 * A utility to safely install the [[http://www.slf4j.org/ slf4j-api]] [[http://www.slf4j.org/api/org/slf4j/bridge/SLF4JBridgeHandler.html SLF4JBridgeHandler]].
 ...
 * dependency present as it may cause an [[http://www.slf4j.org/legacy.html#julRecursion infinite loop]].
 *
 * If the [[http://www.slf4j.org/api/org/slf4j/bridge/SLF4JBridgeHandler.html SLF4JBridgeHandler]] is already installed we do not try to install
 * it again.
 *
 * Additionally note there is a performance impact for bridging jul log messages in this
 * manner. However, using the [[http://logback.qos.ch/manual/configuration.html#LevelChangePropagator Logback LevelChangePropagator]]
 * (> 0.9.25) can eliminate this performance impact.
 *
 * @see [[http://www.slf4j.org/legacy.html#jul-to-slf4j SLF4J: jul-to-slf4j bridge]]
 */
object Slf4jBridgeUtility extends Logging {

  /**
   * Attempt installation of the [[http://www.slf4j.org/ slf4j-api]] [[http://www.slf4j.org/api/org/slf4j/bridge/SLF4JBridgeHandler.html SLF4JBridgeHandler]].
   * The [[http://www.slf4j.org/api/org/slf4j/bridge/SLF4JBridgeHandler.html SLF4JBridgeHandler]] will not be installed if already installed or if the
